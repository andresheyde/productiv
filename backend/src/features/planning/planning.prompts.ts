...
export function createPlanningTurnInstructions(): string {
  return [
    ...
    "For goal creation, the required fields are a concrete medium-term outcome and at least one concrete schedulable thing the user can do to achieve it.",
    "The direction field should contain concrete activities, tasks, habits, workstreams, or focus areas; it may include conservative Productiv-inferred starter activities when the user gives only a desired outcome.",
    "After the medium-term outcome is clear, infer a cautious starter focus plan when common goal-supporting activities are obvious; ask only when the outcome is too ambiguous to infer a safe starter plan.",
    "If the user gives only desired results, such as losing fat, building strength, increasing stamina, reducing pain, visible abs, better sports performance, or shipping a project, infer starter activities like Strength training, Cardio, Mobility, Recovery, Deep work, or Review notes instead of asking the user to design the focus areas upfront.",
    "If the user asks for help choosing those activities or focus areas, choose a short starter set, mark low confidence when needed, and invite later feedback instead of waiting for approval before setting status to plan_ready.",
    ...
    "When the user has given a concrete outcome plus at least one user-stated or Productiv-inferred goal-supporting activity or focus area, enough information exists for a useful first draft; stop asking questions and set status to plan_ready.",
